...
#ifndef _JAG_COND_CPP_ 
#define _JAG_COND_CPP_ 
...
inline float min_t(float x, float y)
...
/* ------------------------------------------------------------------ */
template <typename T>
void Solve_t2(Matrix<T> &a,int  neq, Vector<int> pivot_index, Vector<T> &b)
{
   /*
...
/* ------------------------------------------------------------------ */
template <typename T>
void Solve_t(Matrix<T> &a,int  neq, Vector<int> pivot_index, Vector<T> &b)
{
   /*
    Function Solve solves the linear system A*x=b using the factorization
    obtained from Factor.  Do not use Solve if a zero pivot has
...
//-------------------------------------------------------------------------------------------------
template <typename T >
int Factor_t(Matrix<T> &a, int neq, T *cond, Vector<int> &pivot_index)
{
   /*
    ...
    T anorm, dnorm, ek, t, ynorm;
    int i, j, k, m;
    static T zero = 0.0, one = 1.0;
       Vector<T> temp;
    ...
    */
    ...
    for (k = 0; k < neq-1; k++)
    {
      ...
      for (i = k+1; i < neq; i++)
      {
         if (abs_t(a(i,k)) > abs_t(a(m,k))) m = i;
        }      
      /*
    ...
    */
    ...
    dnorm = zero;
    for (i = 0; i < neq; i++)
      dnorm = max_t(dnorm, abs_t(temp(i)));
    
    Solve_t(a, neq, pivot_index, temp);
    
    ynorm = zero;
    for (i = 0; i < neq; i++)
      ynorm = max_t(ynorm, abs_t(temp(i)));
...
//-------------------------------------------------------------------------------------------------
template <typename T >
int Factor_t2(Matrix<T> &a, int neq, T *cond, Vector<int> &pivot_index)
{
   /*
    Function Factor decomposes the matrix A using Gaussian elimination and
    estimates its condition number;  Factor may be used in conjunction with
