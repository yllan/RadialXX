...
#ifndef _JAG_COND_CPP_ 1
#define _JAG_COND_CPP_ 1


void Solve_d(double **a,int  neq,int * pivot_index, double * b);
int Factor_d(double **a, int neq,double * cond, int *pivot_index);
...
inline double max_d(double x, double y)
...
inline double min_d(double x, double y)
...
/*
  ...
  Declare local variables and initialize:    */
    double anorm, dnorm, ek, t, *temp, ynorm;
    int i, j, k, m;
...
*/
       m = k;
       for (i = k+1; i < neq; i++)
          if (abs_d(a[i][k]) > abs_d(a[m][k])) m = i;
/*
...
*/
    temp = (double*) malloc(neq*sizeof(double));
    ...
    dnorm = zero;
    for (i = 0; i < neq; i++)
       dnorm = max_d(dnorm, abs_d(temp[i]));
    Solve_d(a, neq, pivot_index, temp);
    ynorm = zero;
    for (i = 0; i < neq; i++)
       ynorm = max_d(ynorm, abs_d(temp[i]));
