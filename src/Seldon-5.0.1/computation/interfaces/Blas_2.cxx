...
namespace Seldon
{
	      ...
	      Vector<float, VectFull, Allocator2>& Y)
  {
    ...
    CheckDim(A, X, Y, "MltAdd(alpha, M, X, beta, Y)");
  	...
  	#ifdef RADIAL_WITH_DEBUG
  	  	static int val=0;
  	  	
  	   if(val++==0)
        printf("I am in MltAdd calling cblas_sgemv in file %s near to line %d\n\n",__FILE__,__LINE__);
	      ...
	      Vector<double, VectFull, Allocator2>& Y)
  {
    ...
    CheckDim(TransA, A, X, Y, "MltAdd(alpha, status, M, X, beta, Y)");
  	   ...
  	   if(val++==0)
        printf("I am in MltAdd calling cblas_dgemv in file %s near to line %d\n\n",__FILE__,__LINE__);
