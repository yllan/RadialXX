...
//build the spatial derivative matrices according to:   Ut =  -U*(Ux + Uy) + eps * (Uxx + Uyy)
   fill_derivative(  tps_2d_x , pol_x , rbf ,  c , x , y , A);  
   fill_derivative(  tps_2d_y , pol_y , rbf ,  c , x , y , B);

    Uxy = A + B;  //  Ux + Uy
    
   fill_derivative(  tps_2d_xx , pol_xx , rbf , pol , c , x , y ,  A);
   fill_derivative(  tps_2d_yy , pol_yy , rbf , pol , c , x , y ,  B);
   ...
   //fill_matrix(  tps_2d  , rbf , pol , c , x , y, 0,   ni ,   U);

//       U_n+1    =    U_n + ---
//in rbf
//   H lambda_n+1 =  H lambda_n + --- 
  // fill_gramm(  rbf , pol , c , x , y,  H);
...
/*
 #Para plotear en 3d a archivo
 set terminal png 
 set dgrid3d 40,40,30
 set view 60,30
 set hidden3d
 set dummy u,v
 set parametric
